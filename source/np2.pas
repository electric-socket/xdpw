...
//              ArithOp,CompareOp: (operation: opertype);
...
begin
  ...
  {
function newOp(Ty:NodeType; op: opertype; left: pAstNode): pAstNode;
 var
  node: pAstNode;
 begin
  new(node);
...
//            new(node, number);
    ...
    begin
        ...
        Node^.str.S := '';
        repeat
            Node^.str.S := Node^.str.S + f.s[f.pos];
            inc(f.pos);
            if (f.pos < f.len) and (f.s[f.pos]='''') then
               if f.s[f.pos+1]='''' then   // check for ''
               begin
                   f.pos :=  f.pos + 2;
                   Node^.str.S := Node^.str.S +'''';
               end;
        until (f.s[f.pos] = '''') or EndOf(f);
        if  EndOf(f) or (f.s[f.pos] <> '''') then
           Node^.typ := error
        else
        begin
            Node^.str.Len:= Length(Node^.str.S);
            Node^.str.Pos:= 1;
        end;
    end
...
Procedure WriteOperator(Op:Char);
begin
    Case op of
     '+','-','*','/',
     '<','=','>': Write(Op,' ');
